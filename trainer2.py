...
class Trainer2():
    ...
    def test2(self):
        ...
        self.ckpt.add_log(torch.zeros(1, 5))
        qf = self.extract_feature(self.query_loader2).numpy()
        gf = self.extract_feature(self.test_loader2).numpy()
        ...
        if self.args.re_rank:
            ...
            dist = cdist(qf, gf)
        r = cmc(dist, self.queryset2.ids, self.testset2.ids, self.queryset2.cameras, self.testset2.cameras,
                separate_camera_set=False,
                single_gallery_shot=False,
                first_match_break=True)
